...
OWNER="emkowale"
REPO="frenzy"
PLUGIN_SLUG="frenzy"      # folder name inside the zip
MAIN_FILE="frenzy.php"    # main plugin file (repo root or under PLUGIN_SLUG/)
# ==============================================================================
...
# Automatically remove any tracked-but-deleted files so `git add -A` can't fail.
mapfile -d '' DELETED_FILES < <(git ls-files --deleted -z)
if (( ${#DELETED_FILES[@]} )); then
  step "Clean deleted files"
  git rm --quiet --ignore-unmatch -- "${DELETED_FILES[@]}"
fi
